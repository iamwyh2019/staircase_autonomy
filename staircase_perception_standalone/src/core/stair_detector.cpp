...
StairDetector::StairDetector(stair_utility::StaircaseDetectorParams detector_params, stair_utility::LineExtractorParams line_params)
{
    ...
    verbose_ = detector_params.verbose;
...
bool StairDetector::searchForAscendingStairs(){
    ...
    if(verbose_) {
        ...
        int total_lines = 0;
        std::cout << "\033[1;36m[Ascending] Line distribution by slice:\033[0m" << std::endl;
        for(int ind = 0; ind < detected_lines_above_.size(); ind++){
            int slice_lines = detected_lines_above_[ind]->size();
            if(slice_lines > 0) {
                std::cout << "\033[1;36m  Slice " << ind << ": " << slice_lines << " lines";
                if(ind <= stair_initialization_range_) {
                    std::cout << " [IN INIT RANGE]";
                }
        ...
        std::cout << "\033[1;36m[Ascending] Total lines detected: " << total_lines << "\033[0m" << std::endl;
        std::cout << "\033[1;36m[Ascending] Initialization range (slices): " << stair_initialization_range_ << " (0 to " << stair_initialization_range_ << ")\033[0m" << std::endl;
        std::cout << "\033[1;36m[Ascending] Min stair count: " << min_stair_count_ << "\033[0m" << std::endl;

        // Find first line slice
        int first_line_slice = -1;
        for(int ind = 0; ind < detected_lines_above_.size(); ind++){
            if(detected_lines_above_[ind]->size() > 0) {
                first_line_slice = ind;
                break;
            }
        }
    ...
    while(!stair_detected_ && !is_init_space_empty){
        ...
        if(verbose_) std::cout << "\033[0;36m[Ascending] === Searching for initial stair pair ===\"\033[0m" << std::endl;
        for(int ind = 0; ind < detected_lines_above_.size(); ind++){
            std::shared_ptr<std::deque<stair_utility::DetectedLine>> curr_lines = detected_lines_above_[ind];
            if(ind > stair_initialization_range_){
                if(verbose_) std::cout << "\033[0;33m[Ascending] ✗ Exceeded initialization range (slice " << ind << " > " << stair_initialization_range_ << ") without finding stair pair\033[0m" << std::endl;
                break;
            }
                ...
                if(!line_initialized){
                    ...
                    for(std::deque<stair_utility::DetectedLine>::const_iterator c = curr_lines->begin(); c != curr_lines->end(); ++c){
                        ...
                        if(!line1.skip){
                            ...
                            if(verbose_) {
                                std::cout << "\033[1;32m[Ascending] First line initialized at slice " << ind
                                          << " - center: [" << line1.line_center[0] << ", " << line1.line_center[1]
                                          << ", " << line1.line_center[2] << "], length: " << line1.line_length << "\033[0m" << std::endl;
                            }
                            break;
                        }
                        line1_index++;
                    ...
                    line2_index = 0;
                    if(verbose_) std::cout << "\033[0;36m[Ascending] Checking slice " << ind << " (" << curr_lines->size() << " lines) for pairing with line1 at slice " << initial_step_index << "\033[0m" << std::endl;
                    int candidate_num = 0;
                    for(std::deque<stair_utility::DetectedLine>::const_iterator c = curr_lines->begin(); c != curr_lines->end(); ++c){
                        line2 = *c;
                        candidate_num++;
                        if(!line2.skip){
                            ...
                            if(verbose_) {
                                std::cout << "\033[0;36m[Ascending]   Candidate #" << candidate_num << ": center_diff=" << center_diff
                                          << ", line_diff=" << line_diff << ", angle_diff=" << angle_diff
                                          << ", rel_len_diff=" << relative_length_diff << ", z_diff=" << z_diff
                                          << ", yaw_slope=" << yaw_slope << "\033[0m" << std::endl;

                                // Detailed validation breakdown
                                bool check1 = valid_dist;
                                bool check2 = angle_diff < max_stair_curvature_;
                                bool check3 = relative_length_diff <= 0.3;
                                bool check4 = z_diff >= min_stair_height_ && z_diff <= max_stair_height_;
                                bool check5 = valid_slope;

                                if(!check1) std::cout << "\033[0;33m      FAIL: valid_dist (center: " << center_diff << " not in [" << min_stair_depth_ << "," << max_stair_depth_ << "] OR line: " << line_diff << " not in range OR diff: " << fabs(line_diff - center_diff) << " > 0.05)\033[0m" << std::endl;
                                if(!check2) std::cout << "\033[0;33m      FAIL: angle_diff " << angle_diff << " >= " << max_stair_curvature_ << "\033[0m" << std::endl;
                                if(!check3) std::cout << "\033[0;33m      FAIL: rel_len_diff " << relative_length_diff << " > 0.3\033[0m" << std::endl;
                                if(!check4) std::cout << "\033[0;33m      FAIL: z_diff " << z_diff << " not in [" << min_stair_height_ << "," << max_stair_height_ << "]\033[0m" << std::endl;
                                if(!check5) std::cout << "\033[0;33m      FAIL: yaw_slope " << yaw_slope << " not in [" << stair_slope_min_ << "," << stair_slope_max_ << "]\033[0m" << std::endl;
                            }
...
if(verbose_)                                     std::cout << "\033[1;32m[Ascending] ✓ STAIR PAIR FOUND! Initialized with 2 steps (slices " << initial_step_index << " and " << ind << ")\033[0m" << std::endl;
...
if(verbose_)                                         std::cout << "\033[0;35m[Ascending Ramp] Ramp line detected during initialization (valid dist, small angle): angle_diff=" << angle_diff << ", ramp_count=" << ramp_lines_up_.size() << "\033[0m" << std::endl;
        ...
        if(stair_inited){
if(verbose_)             std::cout << "\033[1;36m[Ascending] Extending staircase from " << stairs_up_.size() << " steps...\033[0m" << std::endl;
                            ...
                            if(valid_dist && angle_diff < max_stair_curvature_ && relative_length_diff <= 0.3 && z_diff >= min_stair_height_ && z_diff <= max_stair_height_ && valid_slope){
                                if(ramp_lines_up_.size() == 0){
                                    ...
                                    if(center_0_diff > center_diff){
                                        stairs_up_.push_back(line2);
if(verbose_)                                         std::cout << "\033[1;32m[Ascending] ✓ Step " << stairs_up_.size() << " added (slice " << ind << ")\033[0m" << std::endl;
                                        break;
                                    }
                                }
                                else{
...
bool StairDetector::searchForDescendingStairs(){
    ...
    if(verbose_) std::cout << "\n\033[1;35m=== DESCENDING STAIR DETECTION START ===\033[0m" << std::endl;
    if(verbose_) std::cout << "\033[1;35m[Descending] Total line slices below ground: " << detected_lines_below_.size() << "\033[0m" << std::endl;

    // Count total lines in all slices
    int total_lines = 0;
    for(int ind = 0; ind < detected_lines_below_.size(); ind++){
        total_lines += detected_lines_below_[ind]->size();
    }
    if(verbose_) std::cout << "\033[1;35m[Descending] Total lines detected: " << total_lines << "\033[0m" << std::endl;
    if(verbose_) std::cout << "\033[1;35m[Descending] Ground index: " << ground_index_ << ", Initialization range: " << stair_initialization_range_ << "\033[0m" << std::endl;
    if(verbose_) std::cout << "\033[1;35m[Descending] Min stair count: " << min_stair_count_ << "\033[0m" << std::endl;
